...
import { BottomSafeProvider, useBottomSafe } from "@/components/BottomSafeContext";
...
function MobileOrderBar() {
  ...
  const { offset, setMobileOrderBar } = useBottomSafe();
  ...
  useEffect(() => {
    setMobileOrderBar(visible && location !== "/contact");
  }, [visible, location, setMobileOrderBar]);

  if (location === "/contact") return null;

  return (
    <div
      className={`md:hidden fixed left-0 right-0 z-[55] transition-transform duration-300 border-t border-white/10 ${visible ? "translate-y-0" : "translate-y-full"}`}
      style={{ backgroundColor: "#0a0e27", bottom: `${offset - 56}px` }}
    >
