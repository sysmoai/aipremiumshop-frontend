...
export function StickyMobileBar({ heroRef }: StickyMobileBarProps) {
  ...
  const { offset, setStickyBar } = useBottomSafe();
  ...
  useEffect(() => {
    setStickyBar(visible);
  }, [visible, setStickyBar]);
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="fixed inset-x-0 z-50 md:hidden border-t border-white/10"
          style={{ backgroundColor: "#0a0e27", bottom: `${offset}px` }}
